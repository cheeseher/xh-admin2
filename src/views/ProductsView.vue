    ...
    >
      <el-form :model="productForm" label-width="100px" :rules="rules" ref="productFormRef">
        ...
        <el-form-item label="单笔购买限制">
          ...
          <div class="form-tip">设置用户单笔订单可购买的最大数量，保障库存合理分配</div>
        </el-form-item>
          ...
          <div v-for="(pkg, index) in packageForm.packages" :key="index" class="package-item">
            ...
            <div class="package-content">
              ...
              <div class="package-right">
                ...
                >
                  <!-- Button to simulate upload if no file is selected and not currently uploading -->
                  <el-button
                    v-if="!pkg.uploading && !pkg.fileName"
                    type="primary"
                    @click="simulateFileUpload(index)"
                  >
                    点击上传文件
                  </el-button>
                  <!-- Tip, visible only when the upload button is visible -->
                  <div v-if="!pkg.uploading && !pkg.fileName" class="el-upload__tip" style="margin-top: 5px;">
                    支持.apk, .ipa, .exe等格式，不超过500MB
                  </div>

                  <!-- Progress bar if currently uploading -->
                  <div v-if="pkg.uploading" style="display: flex; align-items: center; gap: 10px; width: 100%;">
                    <el-progress
                      :percentage="pkg.uploadProgress || 0"
                      status="success"
                      :text-inside="true"
                      :stroke-width="12"
                    ...
                    <el-button 
                      ...
                      size="small" 
                      :icon="Delete" 
                      @click="cancelSimulatedUpload(index)"
                    />
                  </div>

                  <!-- File info if file is "uploaded" (fileName exists) and not currently uploading -->
                  <div v-if="pkg.fileName && !pkg.uploading" class="file-info">
                    <el-icon><Document /></el-icon>
...
const productForm = reactive({
  ...
  packages: [] // Ensure packages are initialized for new products if needed
})
...
const resetProductForm = () => {
  ...
  productForm.packages = [] // Reset packages
}
...
// 上传安装包文件处理
const handleFileUpload = (options: UploadRequestOptions, index: number) => {
  const fileObj = options.file; // Removed type assertion as target is any
  // 验证文件大小
...
const simulateFileUpload = (index: number) => {
  const pkg = packageForm.packages[index];
  pkg.fileName = 'simulated_package.apk'; // Mock file name
  pkg.uploading = true;
  pkg.uploadProgress = 0;

  // Simulate progress
  const interval = setInterval(() => {
    if (pkg.uploadProgress < 100) {
      pkg.uploadProgress += 20; // Increment progress
      if (pkg.uploadProgress > 100) pkg.uploadProgress = 100; // Cap at 100
    } else {
      clearInterval(interval);
      pkg.uploading = false;
      ElMessage.success(`"${pkg.fileName}" 上传完成 (模拟)`);
    }
...
const savePackages = () => {
  ...
  for (let i = 0; i < packageForm.packages.length; i++) {
    ...
    if (pkg.type === 'file' && !pkg.fileName) {
      ElMessage.error(`安装包 #${i + 1} 需要上传文件`)
      return
    }
  }
...
const calculateMaxPurchase = (stock: number, purchaseLimit: any) => {
  if (!purchaseLimit || typeof purchaseLimit.value === 'undefined') {
    return stock; // Or some other default if purchaseLimit is not properly defined
  }
